      ...
      if (view === Views.DAY) {
        try {
          ...
          if (isWeekend) {
            ...
            if (!allowed) {
              setCannotBookMessage('On weekends the appointment must be fully between 9:00 AM and 2:00 PM.');
              return;
            }
          } else {
            const endMinutes = dayjs(end).hour() * 60 + dayjs(end).minute();
    ...
    <Box style={{ display: 'flex', padding: '2rem', gap: '2rem' }}>
      ...
      >
        <Radio.Group name="appointment-type" value={selectedDoctor ?? ''} onChange={setSelectedDoctor}>
          ...
          <Box mb="lg">
            <Title order={3} mb="sm">Last Seen:</Title>
            <Stack spacing="sm">
          ...
          <Box mb="lg">
            <Title order={3} mb="sm">Best Symptom Match:</Title>
            <Stack spacing="sm">
          ...
          <Box mb="lg">
            <Title order={3} mb="sm">Earliest Appointment:</Title>
            <Stack spacing="sm">
      ...
      <Box style={{ width: '75%', display: 'flex', flexDirection: 'column' }}>
        ...
        >
          ...
          {!selectedDoctor && (
            ...
            >
              <Box style={{ background: 'rgba(255,255,255,0.8)', padding: '0.75rem 1rem', borderRadius: 6 }}>
          ...
          <Button
            ...
            onClick={() => {
              if (!selectedAppointment) {
                setCannotBookMessage('No appointment to confirm. Please schedule an appointment first.');
                return;
              }
